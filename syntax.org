...
** (foobar) is not parsed different from foobar
...
The transform (op() f rest*) into (call f rest*) if f resolves to a function (pointer)
and transform into (jux f rest*) else?
