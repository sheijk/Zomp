...
* To be sorted
** TODO [#A] fix cursor positioning on indent
** TODO [#A] fix indent of }
** TODO [#A] a line cannot be uncommented using ctrl-\ if the // does not start at pos 0
** TODO [#A] let toplevel send feedback to emacs + handle errors (abort eval etc.)
** TODO [#A] "" is not parsed as a string
** TODO [#A] macro/template: warnung wenn `(a b c) a/b/c gleichzeitig parameter/lokale vars sind und ohne antiquote verwendet werden
** TODO [#A] eldoc: struct* als name* und nicht als ((type1 field1) (type2 field2)..)* anzeigen
** TODO [#A] framebuffer cleanup/releasing code
** TODO [#A] (op+_d | => zomp eldoc does not work
** TODO [#A] proper error reporting when function/macro is redefined with different parameter count/types
