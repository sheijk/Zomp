...
* To be sorted
** TODO [#A] support functions returning non-void on toplevel

  
** TODO [#A] loops mit broken/exhausted
...
** TODO [#A] indent: ignore parens in strings
** TODO [#A] checked/unchecked vars im type system (checked = auf konsistenz usw. geprueft)
** TODO [#A] type classes: alternative instanz als keyword argument?
...
** TODO [#A] function types: (int,float)->float, float->int, ...
** TODO [#A] operatorsymbol am zeilenanfang/ende = pre/postfix operator fuer ganze zeile?
...
** TODO [#A] oder auch \+\ \+ +\
** TODO [#A] infix/postfix/prefix operatoren: bezeichnen als _+_ _+ +_?
** TODO [#A] exceptions: restartable conditions
** TODO [#A] compilation scheme: base instructions can have effects on the bindings
** TODO [#A] list<int> => template, xml <node> -> xml like trees
** TODO [#A] "foo bar\nend" should be translated correctly
** TODO [#A] GC using macros
