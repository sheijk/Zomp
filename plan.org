...
** TODO [#A] parser: keep track of line and char number and add this to error messages
** TODO [#A] emacs: navigate to error caused by evaluating code in toplevel
** TODO [#A] `(#foo a b c) should turn into (fooValue a b c) instead of (seq fooValue a b c) (at least if foo does not have any arguments)
** TODO [#A] print nice struct names on !bindings (same as on !writeSymbols) in toplevel
...
** TODO [#A] / should be left associative: a / b / c => (a / b) / c
** TODO [#A] (var (ptr float) foo) => better error message instead of expr: /0/
** TODO [#A] change all types to Capitalized case in stdlib
** TODO [#A] fix wrong promts in emacs inferior zomp mode
** TODO [#A] support records as first class var types (allow them to be params, retvals, local vars, global vars)
** TODO [#A] emacs indenting: ignore parens in comments
** TODO [#A] allow immediate execution of code
** TODO [#A] fix initialization of global vars
** TODO [#A] support global pointers															  :zomp:
** TODO [#A] generate more realistic compiler performance tests (use macros and functions which are not defined just two lines ago)
** TODO [#A] remove zompvm.h
** TODO [#A] change ast representation? need a way to distinguish func and (func)															  :zomp:
** TODO [#A] support for const, const T* etc.?
** TODO [#A] Allow labels to be local to a scope? Turn if/for/.. into expressions this way?
** TODO [#A] typechecking fails on (func nonvoid ... (if cond (ret a) (ret b)) )
** TODO [#A] if is not an expression
